...
// Added token argument for localStorage token
function pa11yCi(urls, options, token) {
	return new Promise(async resolve => {
		// Create a test browser to assign to tests
		const testBrowser = await puppeteer.launch(options.chromeLaunchConfig);
		
		//Set localStorage token given as argument -l
		try{
			let page;	
			page = await testBrowser.newPage();
			await page.goto('https://www.norsk-tipping.no', { waitUntil: 'networkidle2' });

			await page.evaluate(
				(token) => {
					localStorage.setItem('ntToken', token)
				}, token
			  );
